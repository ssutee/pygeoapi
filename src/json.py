...
class JsonWriter(object):
    ...
    def _write(self, obj):
        ...
        if isinstance(obj, datetime.datetime):
            self._append('"%s"' % obj.strftime("%s %s" % (self.DATE_FORMAT, self.TIME_FORMAT)))
        elif isinstance(obj, datetime.date):
            self._append('"%s"' % obj.strftime(self.DATE_FORMAT))
        elif isinstance(obj, datetime.time):
            self._append('"%s"' % obj.strftime(self.TIME_FORMAT))
        elif ty is types.DictType:
        	__process_dict(obj)
        # In case we get a Sponty Model that was not converted to dict.
        elif hasattr(obj, 'to_dict'):
        	 __process_dict(obj.to_dict())
	    ...
	    obj = obj.replace('\t', r'\t')
            ...
            raise WriteException, "Cannot write in JSON: %s. Type is: %s" % (repr(obj), ty)
